...
SpaceShip.PlayState = {
  ...
  create() {
    // create player core
    this.playerCore = this.game.add.sprite(
      this.CENTER.x,
      this.CENTER.y,
      this.playerCoreTexture,
    );
    ...
    this.playerShield = this.game.add.sprite(
      this.CENTER.x,
      this.CENTER.y,
      this.playerShieldTexture,
    );
    ...
    this.playerBigShield = this.game.add.sprite(
      this.CENTER.x - this.SHIELD_DIS,
      this.CENTER.y,
      this.BigShieldTexture,
    );
    ...
    // create player gun
    this.playerGun = this.game.add.sprite(
      this.playerCore.x + this.GUN_DIS,
      this.playerCore.y,
      this.playerGunTexture,
    );
    this.playerGun.anchor.setTo(0.5);
    this.playerGun.scale.setTo(0.8);

    // create player double gun
    this.playerDoubleGun = this.game.add.sprite(
      this.playerCore.x + this.GUN_DIS,
      this.playerCore.y,
      this.DoubleGunTexture,
    );
  ...
  update() {
    ...
    let playerGunAngle = this.game.physics.arcade.angleBetween(
      this.playerCore,
      this.playerGun,
    );

    if (
      (pointerAngle - playerGunAngle > this.ANG_TOL / 2 &&
        pointerAngle - playerGunAngle <= Math.PI) ||
      pointerAngle - playerGunAngle < -Math.PI
    ) {
      this.playerGun.angle += this.ANG_VEL;
      this.playerGun.position.rotate(
        this.playerCore.x,
        this.playerCore.y,
        this.ANG_VEL,
        true,
      ...
      this.playerShield.position.rotate(
        this.playerCore.x,
        this.playerCore.y,
        this.ANG_VEL,
        true,
  ...
  shieldRangeCheck(shield, enemyThing) {
    let enemyAngle = this.game.physics.arcade.angleBetween(
      this.playerCore,
      enemyThing,
    );
    let angle =
      shield.angle + 180 > 180 ? shield.angle - 180 : shield.angle + 180;
    ...
    if (
      delta < rng / 2 ||
      delta > Math.PI * 2 - rng / 2 ||
      delta < rng / 2 - Math.PI * 2
    ) {
      return true;
    } else {
      return false;
  ...
  createPlayerBullet() {
    if (this.playerGun.exists) {
      ...
      if (!bullet) {
        bullet = new SpaceShip.PlayerBullet(
          this.game,
          this.playerGun.x,
          this.playerGun.y,
          this.playerBulletTexture,
        );
    ...
    if (this.playerDoubleGun.exists) {
      let dir = this.playerDoubleGun.position
        .clone()
        .rperp()
        .normalize()
        .multiply(20, 20);
  ...
  createLaser() {
    // 取得槍的角度做出單位向量
    let unit = new Phaser.Point(1, 0)
      .rotate(0, 0, this.playerGun.rotation)
      .multiply(this.LASER_DIS, this.LASER_DIS);
  ...
  createUFO() {
    ...
    let position = new Phaser.Point(
      this.playerCore.x + this.SCREEN_DIS,
      this.playerCore.y,
    ).rotate(this.playerCore.x, this.playerCore.y, angle, true);

    if (!ufo) {
      ufo = new SpaceShip.UFO(
        this.game,
        position.x,
        position.y,
        this.ENEMY_SPEED,
  ...
  createMeteor() {
    ...
    if (!meteor) {
      meteor = new SpaceShip.Meteorite(
        this.game,
        position.x,
        position.y,
        this.ENEMY_SPEED,
  ...
  releaseUlt() {
    this.ult.revive();
    this.tweenUlt = this.game.add
      .tween(this.ult.scale)
      .to({ x: 1, y: 1 }, this.ULT_SPEED);
    this.tweenUlt.start();
  ...
  damagePlayer(playerCore, enemyThings) {
    ...
    if (this.currentEndur > this.ENEMY_DAMAGE) {
      ...
      this.tweenEndur = this.game.add
        .tween(this.endurance.scale)
        .to({ x: this.currentEndur }, Phaser.Timer.SECOND);
  ...
  loadLevel() {
    ...
    if (!hasLevel) {
      ...
      this.levelData = JSON.parse(
        this.game.cache.getText(`lv${this.currentLevel}`),
      );
  ...
  scheduleNextEnemy() {
    ...
    if (nextEnemy) {
      let nextTime =
        Phaser.Timer.SECOND * nextEnemy.time -
        (this.currentEnemyIndex === 0
          ? 0
  ...
  // init UIs
  initUI() {
    this.LIFE_INIT_POS = new Phaser.Point(
      this.game.world.width * 0.95,
  ...
  initHeart() {
    ...
    this.HEART_SIZE =
      this.game.cache.getImage('heart').width * this.HEART_SCALE;
    ...
    for (let i = 0; i < this.PLAYER_HEART; i++) {
      let heart = this.game.add.image(
        heartPos.x - this.HEART_SIZE,
        heartPos.y,
        'heart',
      );
  ...
  // init endurance
  initEndurance() {
    let endurPos = this.LIFE_INIT_POS.subtract(
      0,
      this.HEART_SIZE / 2 + this.HEART_SPACE,
    );
    this.endurance = this.game.add.image(
      endurPos.x,
      endurPos.y,
      this.playerEndurTexture,
    );
    ...
    this.game.time.events.loop(
      this.ENDUR_RECOVER_RATE * Phaser.Timer.SECOND,
      () => {
  ...
  initBattery() {
    ...
    this.batteryText = this.game.add.text(
      this.game.world.width * 0.95,
      this.game.world.height * 0.9,
      `${this.batteries} 顆能量電池`,
      style,
    );
  ...
  initCountdown() {
    ...
    let timeText = this.game.add.text(
  ...
  initWaveNotice() {
    ...
    this.waveNotice = this.game.add.text(
      this.game.world.width * 0.5,
      this.game.world.height * 0.2,
